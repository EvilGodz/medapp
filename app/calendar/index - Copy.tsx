...
export default function CalendarScreen() {
  ...
  // ฟังก์ชันตรวจสอบว่ายาควรทานในวันที่กำหนดหรือไม่ตาม dayFrequency
  const shouldTakeMedicationOnDate = (medication: MedRemind, date: Date): boolean => {
    const startDate = new Date(medication.startDate);
    const dayFrequency = medication.dayFrequency || 1;
    
    // คำนวณจำนวนวันที่ผ่านไปตั้งแต่เริ่มทานยา
    const timeDiff = date.getTime() - startDate.getTime();
    const daysDiff = Math.floor(timeDiff / (1000 * 60 * 60 * 24));
    
    // ตรวจสอบว่าวันที่อยู่ในช่วงที่ต้องทานยาหรือไม่
    if (daysDiff < 0) return false; // ยังไม่ถึงวันเริ่มทาน
    
    // ตรวจสอบตาม dayFrequency
    return daysDiff % dayFrequency === 0;
  };
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= days; day++) {
      ...
      // ตรวจสอบว่ามียาที่ต้องทานในวันนี้ตาม dayFrequency หรือไม่
      const hasMedicationsToday = medications.some((medication) => {
        return shouldTakeMedicationOnDate(medication, date);
      });
      ...
      week.push(
        <TouchableOpacity
          ...
          style={[
            ...
            (hasDoses || hasMedicationsToday) && styles.hasEvents,
          ]}
        ...
        >
          ...
          {(hasDoses || hasMedicationsToday) && <View style={styles.eventDot} />}
        </TouchableOpacity>
      );
  ...
  const renderMedicationsForDate = () => {
    ...
    // แสดงเฉพาะยาที่ต้องกินในวันที่ selectedDate ตาม dayFrequency
    const filteredMeds = medications.filter((medication) => {
      const selected = new Date(selectedDate.getFullYear(), selectedDate.getMonth(), selectedDate.getDate());
      
      // ตรวจสอบว่ายาควรทานในวันนี้หรือไม่ตาม dayFrequency
      if (!shouldTakeMedicationOnDate(medication, selected)) {
        return false;
      }
