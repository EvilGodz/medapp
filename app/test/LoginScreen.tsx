...
const LoginScreen: React.FC = () => {
  ...
  const handleLogin = async (): Promise<void> => {
    ...
    try {
      // เรียกใช้ API จริง
      const response = await fetch('http://192.168.1.55:3000/api/auth/login', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        // ⭐ เก็บ token และ user data ใน AsyncStorage
        ...
        Alert.alert('สำเร็จ', data.message, [
          { text: 'ตกลง', onPress: () => router.replace('/test/HomeScreen') }
        ]);
      } else if (data.emailVerified === false) {
        // 🆕 ผู้ใช้ยังไม่ได้ยืนยันอีเมล
        Alert.alert(
          'ต้องยืนยันอีเมล',
          data.message,
          [
            ...
            { 
              text: 'ส่งอีเมลใหม่', 
              onPress: () => router.push({ pathname: '/test/EmailVerificationScreen', params: { email: data.email || email } })
            }
          ]
        );
      } else {
    ...
    >
      <View style={styles.formContainer}>
        ...
        <View style={styles.signupContainer}>
          ...
          <TouchableOpacity onPress={() => router.push('/test/SignupScreen')}>
