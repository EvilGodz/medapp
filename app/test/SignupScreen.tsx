...
import { SignupFormData, AuthResponse } from '../../types/types';
...
export default function SignupScreen () {
  ...
  const handleSignup = async (): Promise<void> => {
    ...
    try {
      ...
      // เรียกใช้ API จริง
      const response = await fetch('http://192.168.1.55:3000/api/auth/signup', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        // ไม่เก็บ token ทันที เพราะต้องยืนยันอีเมลก่อน
        Alert.alert('สำเร็จ', data.message, [
          { 
            text: 'ตกลง', 
            onPress: () => router.push({ pathname: '/test/EmailVerificationScreen', params: { email: formData.email, fullname: formData.fullname } })
          }
        ]);
      } else {
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.formContainer}>
          ...
          <View style={styles.loginContainer}>
            ...
            <TouchableOpacity onPress={() => router.push('/test/LoginScreen')}>
